        ...
        if(!req.body.title) {
            ...
            if(!req.body.body) {
                ...
                if(!req.body.createdBy) {
                    res.json({ success: false, message: 'Blog creator is required.' }); // Return error message 
                } else {
                            ...
                            if(err.errors){
                                ...
                                if(err.errors.title) {
                                    ...
                                    if(err.errors.body) {
                                         ...
                                         res.json({ success: false, message: err.errmsg }); // Return general error message
                                    }
                                }
                            } else {
                            ...
                            if(err) {
                                ...
                                if(!user) {
                                    ...
                                    if(user.username !== blog.createdBy) {
                                        res.json({ success: false, message: 'You are not authorize to edit this blog post.' }); // Return authentication reror
                                    } else {
                        ...
                        User.findOne({ _id: req.decoded.userId }, (err, user) => {
                            ...
                            if(err) {
                                ...
                                if(!user) {
                                    ...
                                    if(user.username !== blog.createdBy) {
                                        res.json({ success: false, message: 'You are not authorize to edit this blog post.' }); // Return error message
                                    } else {
                                        blog.title = req.body.title;    // Save latest blog title
                                        blog.body = req.body.body;  // Save latest body
                                        // Save the blog from database
                                        blog.save((err) => {
                                            if(err) {
                                                if(err.errors) {
                                                    res.json({ success: false, message: 'Please ensure form is filled out properly' }); // Return error message
                                                } else {
                                                    res.json({ success: false, message: err }); // Return error message
                                                } 
                                            } else {
                        ...
                        User.findOne({ _id: req.decoded.userId }, (err, user) => {
                            ...
                            if(err) {
                                ...
                                if(!user) {
                                    ...
                                    if(user.username !== blog.createdBy) {
                                        res.json({ success: false, message: 'You are not authorize to delete this blog post' });    // Return error message
                                    } else {
